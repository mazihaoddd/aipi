...
class TrainValDataset(Dataset):
    ...
    def __init__(self, img_dir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, check_images=False, check_labels=False, stride=32, pad=0.0, rank=-1, class_names=None, task='train'):
    ...
    def get_imgs_labels(self, img_dir):
        ...
        if not img_info:
            with open(valid_img_record, 'r') as f:
                ...
                assert img_info, 'No information in record files, please add option --check_images.'
        img_paths = list(img_info.keys())
        label_paths = [osp.join(label_dir, osp.basename(p).split('.')[0]+'.txt') for p in img_paths]
        if self.check_labels or 'labels' not in img_info[img_paths[0]]:  # key 'labels' not saved in img_info
